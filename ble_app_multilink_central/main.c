...
#define BEACON_NUM                2
#define FILTER_FRAME_SIZE         20

static const char m_target_periph_name[] = "My_Beacon";                  /**< Name of the device we try to connect to. This name is searched for in the scan report data*/
...
// static int8_t rssi_frame[BEACON_NUM][FILTER_FRAME_SIZE] = {{0}, {0}};
// static uint8_t frame_idx[BEACON_NUM] = {0};

static struct Beacons {
    ble_gap_addr_t mac_addr;
    int8_t rssi_frame[FILTER_FRAME_SIZE];
    uint8_t frame_idx;
};

static struct Beacons beacons[BEACON_NUM];
int beacons_num = 0;

// int16_t average_filter(uint16_t beacon_idx){
//     uint8_t idx;
//     int16_t avg_rssi = 0;

//     for(idx = 0; idx < FILTER_FRAME_SIZE; idx++)
//         avg_rssi += rssi_frame[beacon_idx][idx];

//     avg_rssi /= FILTER_FRAME_SIZE;

//     return avg_rssi;
// }

float average_filter(uint16_t beacon_idx){
    uint8_t idx;
    float avg_rssi = 0.0;

    for(idx = 0; idx < FILTER_FRAME_SIZE; idx++)
        avg_rssi += beacons[beacon_idx].rssi_frame[idx];
    ...
    if (found_name)
    {
        if (strlen(m_target_periph_name) != 0)
        {
            if (memcmp(m_target_periph_name, dev_name.p_data, dev_name.size) == 0)
            {
                 ...
                 //            peer_addr->addr[5]
                 //            );
                for (idx = 0; idx < beacons_num; idx++)
                {
                    if (memcmp(&(beacons[idx].mac_addr), peer_addr, 6) == 0)
                    {
                        // NRF_LOG_INFO("Beacon[%d] found\r\n", idx);
                        beacon_idx = idx;
                        break;
                    }
                }
                ...
                if (idx == beacons_num)
                {
                    if (beacons_num == BEACON_NUM)
                    {
                        NRF_LOG_ERROR("Beacons is full\r\n");
                    }
                    else
                    {
                        memcpy(&(beacons[beacons_num].mac_addr), peer_addr, 6);
                        NRF_LOG_INFO("New Address %02x%02x%02x%02x%02x%02x\r\n",
                                beacons[beacons_num].mac_addr.addr[0],
                                beacons[beacons_num].mac_addr.addr[1],
                                beacons[beacons_num].mac_addr.addr[2],
                                beacons[beacons_num].mac_addr.addr[3],
                                beacons[beacons_num].mac_addr.addr[4],
                                beacons[beacons_num].mac_addr.addr[5]
                                );
                        beacon_idx = beacons_num;
                        beacons_num++;
                    }
                }
                ...
                if (beacon_idx >= 0)
                {
                    uint8_t frame_idx = beacons[beacon_idx].frame_idx;

                    rssi = p_gap_evt->params.adv_report.rssi;

                    // NRF_LOG_INFO("RSSI: %d\r\n", rssi);

                    beacons[beacon_idx].rssi_frame[frame_idx++] = rssi;
                    frame_idx %= FILTER_FRAME_SIZE;
                            
                    filter_rssi = average_filter(beacon_idx);
                    NRF_LOG_INFO("RSSI[%d]: %d\r\n", beacon_idx, filter_rssi);
                }
...
static void on_ble_evt(const ble_evt_t * const p_ble_evt)
{
    ...
    // int16_t idx;
    const ble_gap_addr_t  * const peer_addr  = &p_gap_evt->params.adv_report.peer_addr;
    static int16_t beacon_idx;
    static int16_t filter_rssi;
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GAP_EVT_RSSI_CHANGED:
        {
            ...
            beacon_idx = p_gap_evt->conn_handle;            

            rssi = p_ble_evt->evt.gap_evt.params.rssi_changed.rssi;

            NRF_LOG_INFO("RSSI: %d\r\n", rssi);

            beacons[beacon_idx].rssi_frame[beacons[beacon_idx].frame_idx++] = rssi;
            beacons[beacon_idx].frame_idx %= FILTER_FRAME_SIZE;
