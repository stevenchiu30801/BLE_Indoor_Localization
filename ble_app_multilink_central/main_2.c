...
#define FILTER_FRAME 10
#define ANCHOR_LENGTH 4

float rssi_frame[ANCHOR_LENGTH][FILTER_FRAME] = {{0},{0}};
float rssi_avgs[ANCHOR_LENGTH] = {0};
uint8_t frame_idx[ANCHOR_LENGTH] = {0};
uint8_t coord[ANCHOR_LENGTH][2] = {{0},{0}};

static const char m_target_periph_name[] = "Zero";  
static const char m_target_periph_name1[] = "One"; 
static const char m_target_periph_name2[] = "Two";                 /**< Name of the device we try to connect to. This name is searched for in the scan report data*/
...
static void assign_rssi(int bIndx, int8_t rssi) {
	rssi_frame[bIndx][frame_idx[bIndx]++] = rssi;
	if(frame_idx[bIndx] >= FILTER_FRAME)
			frame_idx[bIndx] = 0;
		float avg_rssi = 0;
		float sum_rssi = 0;
		for(int i = 0; i < FILTER_FRAME; i++) {
			sum_rssi = sum_rssi + rssi_frame[bIndx][i];
		}
		avg_rssi = sum_rssi/FILTER_FRAME*10;
		rssi_avgs[bIndx] = avg_rssi;
		NRF_LOG_INFO("   avg[%d]: %d \r\n", bIndx, avg_rssi);
}
...
static void on_adv_report(const ble_evt_t * const p_ble_evt)
{
    ...
    adv_data.size   = p_gap_evt->params.adv_report.dlen;
			...
			//NRF_LOG_INFO("FOUND DEVICE: %s\r\n",(uint32_t)dev_name.p_data);
    ...
    if (found_name)
    {
        if (strlen(m_target_periph_name) != 0)
        {
            ...
            {
                assign_rssi(0, RSSI);
            } else if(memcmp(m_target_periph_name1, dev_name.p_data, dev_name.size) == 0){
                assign_rssi(1, RSSI);
						} else if(memcmp(m_target_periph_name2, dev_name.p_data, dev_name.size) == 0){
                assign_rssi(2, RSSI);
						}
...
/**This function will assign coordinates based off of the mac address of the device

*/

static void assign_coords(const ble_gap_evt_adv_report_t *p_adv_report, int8_t b_idx)
{
		uint8_t mac_addr[2][6] = {{0x26,0xff,0x01,0x88,0x0a,0xc6}, {0x16,0xce,0x14,0x76,0xfc,0xf3}};
		for(int b = 0; b < ANCHOR_LENGTH; b++) {
			for(int k = 0; k < 6; k++) {
				if(p_adv_report->peer_addr.addr[k] != mac_addr[b][k])
						break;
				else if(k == 5) {
					NRF_LOG_INFO("FOUND!! \r\n");
					switch(b) {
		...
		NRF_LOG_INFO("CONN HANDLE-%x:    %02x%02x%02x%02x%02x\r\n",b_idx,
...
static void on_ble_evt(const ble_evt_t * const p_ble_evt)
{
    ...
    const ble_gap_evt_t * const p_gap_evt = &p_ble_evt->evt.gap_evt;
		...
		float avg_rssi = 0;
		float sum_rssi = 0;
		int8_t bIndx = p_gap_evt->conn_handle; 
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GAP_EVT_CONNECTED:
        {
            ...
            err_code = ble_db_discovery_start(&m_ble_db_discovery[p_gap_evt->conn_handle],
						...
						assign_coords(&p_gap_evt->params.adv_report, bIndx);
            ...
            sd_ble_gap_rssi_start(p_ble_evt->evt.gap_evt.conn_handle, 5, 4);
...
int main(void)
{
    ...
    APP_ERROR_CHECK(err_code);
    NRF_LOG_INFO("Multilink Example123\r\n");
    leds_init();
